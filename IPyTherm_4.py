...
def cargar_Datos():
    f = pd.read_excel(nameData)
    f.head()
    data2 = pd.DataFrame(f)
    data2 = data2.set_index('Name')
    data2 = data2.ix[:, 1:12]
    Etiquetas = data2.index.get_values()
    print("Los datos del archivo: {0}, se han cargado correctamente !!!".format(nameData))
    return Etiquetas
...
class Fugacidad():
    ...
    #    if self.nC == 1:
    #        self.D_iT = 2 * np.sum(self.ni * self.da_iidT)
    #    else:
    #        self.D_iT = 2 * np.sum(self.ni * self.daijdT)
            
    #    self.D_T = 0.5 * np.sum(self.ni * self.D_iT)
    ...
    def parametro_D(self):
        if self.nC == 1:
            self.D = self.ni ** 2 * self.a_ii
            self.Di = 2 * self.ni * self.a_ii
            self.DiT = 2 * np.sum(self.ni * self.da_iidT)
            self.DT = 0.5 * np.sum(self.ni * self.DiT)        
        else:
            di = np.ones((len(self.ni), len(self.ni)))
            self.Di = np.ones((len(self.ni)))
            self.D = np.ones((len(self.ni)))
            for i in range(self.nC):
                for j in range(self.nC):
                    ...
                    self.Di[i] = 2 * np.sum(di[i, :])
            self.D = 0.5 * np.sum(self.ni * self.Di)
            self.DiT = 2 * np.sum(self.ni * self.daijdT)
            self.DT = 0.5 * np.sum(self.ni * self.DiT)
    ...
    def cal_entropy(self):
        ...
        self.FT = self.D / self.T ** 2 * self.f
        self.FD = - self.f / self.T
        
        self.dFdT = self.FT + self.FD * self.DT   
        
        self.g_r = np.log(1 - self.B / self.V)
        
        self.f_r = np.log((self.V + self.s1 * self.B) / (self.V + self.s2 * self.B)) / (self.R * self.B) / (self.s1 - self.s2)
        
        self.F = -self.nT * self.g - (self.D/self.T)  * self.f_r
        
        self.entropy = -self.T * self.dFdT - self.F
    ...
    def cal_termodinamica_residual(self):
    ...
    def fluido(self):
        ...
        #self.ter_residual = self.cal_termodinamica_residual()
        #print ("entropy = {0}".format(self.ter_residual))
        return self.Fug
...
def calculaFugacidad(x, Pe, nif, nCf, eq, TcDato, PcDato, wDAto, Avsl):
    ...
    #fugacidad.cal_termodinamica_residual()
