...
class Thermodynamic_correlations(object):
	...
	def fun_ther(self, property_thermodynamics):
	...
	def property_cal(self, component, property_thermodynamics, temperature = None):
		...
		thermo = self.fun_ther(property_thermodynamics)

		#select_constans = [x + property_thermodynamics[3] for x in range(0, 13*size_data, 13)]
		select_constans = [x + thermo[3] for x in range(0, 13*size_data, 13)]
		
		values_constans = self.read_dppr().ix[select_constans, 1:8].get_values()
		table_constans = pd.DataFrame(data=values_constans,index=self.data_name_cal(),
