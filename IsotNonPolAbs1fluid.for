...
C
	...
	ELSE
		...
		READ (*,*) TEM
c	    WRITE (*,*) 'ENTER CRITICAL TEMPERATURE Tc'
c		READ (*,*) TC
		Tr=TEM/Tc
	END IF
...
c	Subcritical isotherms
 10	PVini=0.0020 !0.2this INITIAL VALUES for reduced variables are OK for Tr=0.8
	RHOL=2.30
	RHOV=0.08
	RHOL=2.90
	RHOV=0.002
...
c
	...
	zeta=1.0D0+s1*d+2*c(4)*d**2/Tr**t(4)+3*c(5)*d**3/Tr**t(5)
	zeta=zeta+7*c(6)*d**7/Tr**t(6)
		do k=7,12
...
c
	...
	ra=s1*d+c(4)*d**2/Tr**t(4)+c(5)*d**3/Tr**t(5)+c(6)*d**7/Tr**t(6)
		do k=7,12
		ra=ra+c(k)*frho(k)
		end do
	end
